...
for experiment in experiment_list:
    ...
    if len(paired_df)>0:
    	...
    	os.system("parallel -j2 --colsep '\\t' -a process.tsv bash workflow.rnaseq.paired.double.sh {1} {2} {3} {4} {5} {6} {7}")
    
    if len(single_df)>0:
    	single_df = pd.DataFrame(single_df)
    	single_df.to_csv("process.tsv", sep="\t", index=None, header=None)
    	print("Starting to run {}. Done: {} Remaining: {}".format(experiment, len(done_list), len(paired_df)+len(single_df)))
    	os.system("parallel -j2 --colsep '\\t' -a process.tsv bash workflow.rnaseq.single.double.sh {1} {2} {3} {4} {5}")